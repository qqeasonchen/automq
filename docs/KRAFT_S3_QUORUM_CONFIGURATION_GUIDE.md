...
### S3 存储大小配置格式
AutoMQ 的 S3 Stream 配置项需要使用**字节数值**而不是带单位的字符串：

| 配置项 | ❌ 错误格式 | ✅ 正确格式 | 说明 |
|--------|-------------|-------------|------|
| `s3.wal.cache.size` | `64MB` | `67108864` | 64 * 1024 * 1024 |
| `s3.wal.upload.threshold` | `32MB` | `33554432` | 32 * 1024 * 1024 |
| `s3.block.cache.size` | `512MB` | `536870912` | 512 * 1024 * 1024 |
| `s3.stream.split.size` | `8MB` | `8388608` | 8 * 1024 * 1024 |
| `s3.object.block.size` | `2MB` | `2097152` | 2 * 1024 * 1024 |
| `s3.network.baseline.bandwidth` | `100MB` | `104857600` | 100 * 1024 * 1024 |
...
### Quorum 配置项格式
AutoMQ S3 Quorum Storage 配置项使用带单位的字符串格式：

| 配置项 | ✅ 正确格式 | 说明 |
|--------|-------------|------|
| `automq.s3.wal.cache.size` | `200MB` | 支持单位格式 |
| `automq.s3.quorum.write.cache.size.mb` | `512` | 数值（已含单位） |

## 故障排除

### 常见问题
1. **配置格式错误**: 
   - 错误：`Invalid value 64MB for configuration s3.wal.cache.size: Not a number of type LONG`
   - 解决：使用纯数字格式，如 `67108864` 而不是 `64MB`
2. **WAL 权限错误**: 确保 S3 访问权限正确配置
3. **副本连接失败**: 检查网络连接和端点配置
4. **性能问题**: 调整缓存大小和并发配置
5. **存储桶访问**: 验证 S3 认证和桶权限
